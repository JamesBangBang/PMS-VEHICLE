...
function iniDataTable(){
    mainTable = $('#parkingSituation').DataTable( {
        ordering: false,
        processing:true,
        searching:false,
        sScrollY:"300px",
...
function searchParkingSituation() {
    var params = {
        "beginTime": $("#start").val(),
        "endTime": $("#end").val(),
...
function exportParkingSituation() {
    location.href = starnetContextPath + "/report/getParkingSituationExcel?sheetName=停车情况统计报表&beginTime="
        + $("#start").val() + "&endTime="+$("#end").val() + "&reportType=" + reportType + "&isExistDuration=0";
}
