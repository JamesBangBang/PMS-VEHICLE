...
function iniDataTable(){
    mainTable = $('#attributeCharge').DataTable( {
        ordering: false,
        processing:true,
        searching:false,
        sScrollY:"300px",
        ...
        paging:true,
        ajax: {
            url: starnetContextPath + "/report/getAttributeChargeReport",
            type: 'POST',
            data:{
                "beginTime":"",
                "endTime": "",
...
function searchAttributeCharge() {
    var carAttribute = $("#carAttribute").val();
    var params = {
        "beginTime": $("#start").val(),
        "endTime": $("#end").val(),
...
function exportAttributeCharge() {
    var carAttribute = $("#carAttribute").val();
    location.href = starnetContextPath + "/report/getAttributeChargeExcel?sheetName=车辆属性收费分析表&beginTime="
